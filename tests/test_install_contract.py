...
class MihomoInstallContractTest(unittest.TestCase):
    def test_one_click_installer_exists(self):
        ...
        self.assertIn("write_env_line()", text)
        self.assertIn('write_env_line "WEB_SESSION_SECRET" "$session_secret"', text)
        self.assertIn('write_env_line "MIHOMO_PANEL_REPO_URL" "$REPO_URL"', text)
