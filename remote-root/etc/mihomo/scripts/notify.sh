...
if [[ "$NOTIFY_TG" == "true" && -n "$TG_BOT_TOKEN" && -n "$TG_CHAT_ID" ]]; then
    ...
    TG_CODE=$(curl -sS -m 20 -o "$TG_OUT" -w "%{http_code}" -X POST "https://api.telegram.org/bot${TG_BOT_TOKEN}/sendMessage" \
        -d chat_id="${TG_CHAT_ID}" \
        -d text="${FULL_TEXT}" \
        -d parse_mode="HTML")
    TG_EXIT=$?
    if [[ "$TG_EXIT" -ne 0 || "$TG_CODE" -lt 200 || "$TG_CODE" -ge 300 ]]; then
        log_notify "Telegram failed title=${TITLE} exit=${TG_EXIT} http=${TG_CODE} response=$(cat "$TG_OUT" 2>/dev/null)"
    else
...
if [[ "$NOTIFY_API" == "true" && -n "$NOTIFY_API_URL" ]]; then
    ...
    if [[ "$API_EXIT" -ne 0 || "$API_CODE" -lt 200 || "$API_CODE" -ge 300 ]]; then
        log_notify "Webhook failed title=${TITLE} exit=${API_EXIT} http=${API_CODE} response=$(cat "$API_OUT" 2>/dev/null)"
    else
